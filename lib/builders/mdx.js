...
export default async (modulePath, props) => {
  ...
  if (component.layout) {
    ...
    let layoutPath =
      (component.frontmatter && component.frontmatter.layout) || (data.data && data.data.layout)
    let parts = modulePath.split(props.sourceDir)
    layoutPath = `${parts[0]}/${props.sourceDir}/${layoutPath}`
    if (layoutPath) {
